...
extern bool isAI; // Ce bool sera à true si le joueur joue contre l'ordinateur, ou à false si ce n'est pas le cas. Si vous le pouvez, faites un mode contre l'ordi dans votre jeu qui utilise ce bool
...
void blackjack::distribution (int joueur[], int ordi[], int & choix, int & tour, int & somme, int & partie, std::string prenom) {
	...
	if ( somme == 21 ) {
		...
		if (choix == 1)  {
					...
					init(joueur, 10);
					init(ordi, 10);
					distribution(joueur,ordi,choix,tour,somme,partie,prenom);
		}
	...
	if (sommeordi >= seuil) {
		...
		std::cout<<"  "<<std::endl;
		std::cout<<" Voulez-vous continuer 1 = Oui  0 = Non  "<<std::endl;
		std::cin>>choix;
	}
...
void blackjack::demarrer (int joueur[],int ordi[], int & tour, int & choix, int & somme, int & partie, std::string prenom) {
	tour=0;

	std::cout<<" Êtes-vous prêt ? 1 = Oui  0 = Non  "<<std::endl;
	std::cin>>choix;
		...
		if (choix==1) {
			...
			demarrer(joueur, ordi, tour,choix,somme,partie,prenom); 

		}
}

int blackjack::main() {
	clearConsole(consS); // Vider la console

	std::cout << "Blackjack - Ryan" << std::endl;
	getConsolePos(&consS);

	// std::cout << "Score / Autre truc utile à écrire qui peut être mis à jour" << std::endl; // Supprimer si pas besoin
	// getConsolePos(&consP);
	...
	std::cout<<" Voulez vous lire les règles ? 1 = Oui  0 = Non " <<std::endl;
	std::cin>>choix;
